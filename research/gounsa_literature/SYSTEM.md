...
새 논문 파일을 만들기 전에 반드시 다음 순서로 기존 `papers/`를 확인한다.

1. DOI가 있으면 DOI를 먼저 검색한다.
2. DOI가 없으면 정확한 논문 제목을 검색한다.
3. 그래도 불확실하면 `연도 + 제1저자`가 같은 파일을 확인한다.
4. 같은 DOI 또는 동일 서지의 논문이 이미 있으면 새 파일을 만들지 않고 기존 canonical 파일을 갱신한다.
5. 같은 저자와 연도라는 이유만으로 서로 다른 논문을 합치지 않는다.
6. 중복본에만 있는 식, 단위, 고운사 적용판정, coupling 설명이 있으면 canonical 파일에 병합한 뒤 중복 파일을 삭제한다.
7. 중복 통합 후 `INDEX.md`를 실제 트리 기준으로 다시 생성한다.
8. 대규모 중복 정리는 `decisions/`에 감사 기록을 남긴다.
