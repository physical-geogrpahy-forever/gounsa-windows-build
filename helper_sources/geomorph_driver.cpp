...
int main(int argc,char**argv){try{
 ...
 // Actual daily process sensitivity: published COPLAS transfer values + fixed validated Gounsa D8, finite regolith
 ...
 for(int di=0;di<ND;++di){
   ...
   // conservative runoff accumulation on fixed D8
   ...
   double exf=0,exd=0; // supply-limited fluvial transport
   for(int i:order){double avail=soilmass[i]+incoming[i];double outm=std::min(prop[i],std::max(0.0,avail));net[i]+=incoming[i]-outm;int r=C[i].rec;if(r<0){/* outlet itself fixed */}else if(r==outlet)exf+=outm;else incoming[r]+=outm;}
   for(int i=0;i<N;++i)soilmass[i]+=net[i];
   // supply-limited diffusion on same link network
 ...
 // annual ecological/catchment output
